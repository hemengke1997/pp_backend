import Layout from '@/layout';

 // 官网管理
const officeRouter = {
    path:'/office',
    component: Layout,
    redirect:'/office/recommendation/recommendclass',
    name:'Office',
    meta:{
        title:'官网管理',
        icon:'component'
    },
    children:[
      {
        path:'recommendation',
        component:()=>import('@/views/office/recommendation/index'),
        redirect:'/office/recommendation/class',
        name:'Recommendation',
        meta:{title:'运营推荐'},
        children:[
            {
                path:'class',
                component:()=>import('@/views/office/recommendation/class'),
                name:'Class',
                meta:{title:'推荐分类'}
            },
            {
                path:'content',
                component:()=>import('@/views/office/recommendation/content'),
                name:'Content',
                meta:{title:'推荐内容'}
            }
        ]
      },
      {
        path:'postnews',
        component:()=>import('@/views/office/postnews/index'),
        name:'Postnews',
        // redirect:
        meta:{title:'公告新闻'},
        children:[
            {
                path:'newstype',
                component:()=>import('@/views/office/postnews/newstype'),
                name:'newsType',
                meta:{title:'新闻类型配置'}
            },
            {
                path:'newscontent',
                component:()=>import('@/views/office/postnews/newscontent'),
                name:'newsContent',
                meta:{title:'新闻内容'}
            },
            {
                path:'postcontent',
                component:()=>import('@/views/office/postnews/postcontent'),
                name:'postContent',
                meta:{title:'公告内容'}
            },
            {
                path:'bannercontent',
                component:()=>import('@/views/office/postnews/bannercontent'),
                name:'bannerContent',
                meta:{title:'banner内容'}
            },
        ]
      },
      {
        //游戏攻略   
        path:'strategy',
        component:()=>import('@/views/office/strategy/index'),
        name:'Strategy',
        meta:{title:'游戏攻略'},
        children:[
            {
                path:'class',
                component:()=>import('@/views/office/strategy/class'),
                name:'Class',
                meta:{title:'攻略分类'}
            },
            {
                path:'label',
                component:()=>import('@/views/office/strategy/label'),
                name:'Label',
                meta:{title:'攻略标签'}
            },
            {
                path:'content',
                component:()=>import('@/views/office/strategy/content'),
                name:'Content',
                meta:{title:'攻略内容'}
            },
        ]
      },
      {
        path:'goods',
        component:()=>import('@/views/office/goods/index'),
        name:'Goods',
        meta:{title:'商品管理'},
        children:[
            {
                path:'label',
                component:()=>import('@/views/office/goods/label'),
                meta:{title:'标签'}
            },
            {
                path:'class',
                component:()=>import('@/views/office/goods/class'),
                meta:{title:'分类'}
            },
            {
                path:'tools',
                component:()=>import('@/views/office/goods/tools'),
                meta:{title:'道具'}
            },
            {
                path:'role',
                component:()=>import('@/views/office/goods/role'),
                meta:{title:'角色'}
            },
        ]
      }
    ]
}
export default officeRouter